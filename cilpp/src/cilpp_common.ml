...
let debug_println level str = if level > debug_level then ()
    else (output_string Stdlib.stderr (str ^ "\n"); flush Stdlib.stderr)
    ...
    match fork () with
        | 0 -> (try execvp (List.hd argvList) (Array.of_list argvList)
            with Unix_error(err, _, _) ->
                output_string Stdlib.stderr ("cannot exec the " ^ cmdFriendlyName ^ " command (`" ^ (List.hd argvList) ^ "'): " ^ (error_message err) ^ "\n");
                exit 255
          )
        | childPid ->
...
let prepareCilFile ?(printOnlyCABS=false) argArr =
    ...
    let _ = output_string Stdlib.stderr ("Loading " ^ (string_of_int (List.length ppPluginsToLoad)) ^ " plugins=features\n") in
    ...
    List.iter (fun plugin -> 
        (output_string Stdlib.stderr ("Loading CIL feature %s" ^ plugin ^ "\n") ;
    ...
    List.iter
      (fun fdesc -> 
        if fdesc.Feature.fd_enabled then begin
          ...
          with Not_found -> (output_string Stdlib.stderr ("CIL pass " ^ fdesc.Feature.fd_name ^ " raised Not_found!\n"); raise Not_found);
